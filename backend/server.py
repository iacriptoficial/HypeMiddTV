        ...
        if order_executed:
            ...
            if tp1_price or tp1_perc:
                ...
                try:
                    ...
                    formatted_tp_price = format_price_for_symbol(tp1_target, symbol)
                    ...
                    # CORRECT STRUCTURE: Take Profit Limit Order using proper Hyperliquid syntax
                    tp1_order_result = exchange.order(
                        ...
                        order_type={
                            "trigger": {
                                "price": formatted_tp_price,    # ✅ CORREÇÃO: "price" em vez de "triggerPx"
                                "is_market": False,             # ✅ CORREÇÃO: Explicitamente Limit execution
                                "trigger": "tp"                 # ✅ CORREÇÃO: "trigger" em vez de "tpsl"
                            }
                        },
            ...
            if tp2_price or tp2_perc:
                ...
                try:
                    ...
                    # Place TP2 order using trigger order type (attempt 1: remove isMarket)
            ...
            if tp3_price or tp3_perc:
                ...
                try:
                    ...
                    # Truncate TP price to ensure it never exceeds the original strategy price
                    formatted_tp_price = format_price_for_symbol(tp3_target, symbol)
                    
                    await log_message("INFO", f"🎯 Placing TP3: {'BUY' if tp_is_buy else 'SELL'} {tp3_size} {symbol} at ${formatted_tp_price} (price truncated from ${tp3_target})")
                    
                    # Place TP3 order using trigger order type (attempt 1: remove isMarket)
            ...
            if tp4_price or tp4_perc:
                ...
                try:
                    ...
                    # Truncate TP price to ensure it never exceeds the original strategy price
                    formatted_tp_price = format_price_for_symbol(tp4_target, symbol)
                    
                    await log_message("INFO", f"🎯 Placing TP4: {'BUY' if tp_is_buy else 'SELL'} {tp4_size} {symbol} at ${formatted_tp_price} (price truncated from ${tp4_target}) (COMPLETE EXIT)")
                    
                    # Place TP4 order using trigger order type (attempt 1: remove isMarket)
