...
def reset_uptime_stats():
    """Reset uptime statistics (but keep monitoring start time)"""
    ...
    # Keep monitoring_start_time unchanged to track total monitoring period
        ...
        if order_executed:
            ...
            if stop_price:
                ...
                try:
                    ...
                    await log_message("INFO", f"🛑 Placing stop loss: {'BUY' if stop_is_buy else 'SELL'} {quantity} {symbol} at ${formatted_stop_price} (original: ${stop_price}, formatted with pxDecimals: {px_decimals})")
            ...
            if tp1_price or tp1_perc:
                ...
                try:
                    ...
                    await log_message("INFO", f"🎯 Placing TP1: {'BUY' if tp_is_buy else 'SELL'} {tp1_size} {symbol} at ${formatted_tp_price} (price formatted with pxDecimals: {px_decimals}, value: ${order_value:.2f})")
            ...
            if tp2_price or tp2_perc:
                ...
                try:
                    ...
                    # Format TP price using pxDecimals for precise price formatting
                    formatted_tp_price = format_price_with_px_decimals(tp2_target, px_decimals)
                    ...
                    await log_message("INFO", f"🎯 Placing TP2: {'BUY' if tp_is_buy else 'SELL'} {tp2_size} {symbol} at ${formatted_tp_price} (price formatted with pxDecimals: {px_decimals}, value: ${order_value:.2f})")
            ...
            if tp4_price or tp4_perc:
                ...
                try:
                    ...
                    # Format TP price using pxDecimals for precise price formatting
                    formatted_tp_price = format_price_with_px_decimals(tp4_target, px_decimals)
                    ...
                    await log_message("INFO", f"🎯 Placing TP4: {'BUY' if tp_is_buy else 'SELL'} {tp4_size} {symbol} at ${formatted_tp_price} (price formatted with pxDecimals: {px_decimals}, value: ${order_value:.2f}) (COMPLETE EXIT)")
...
async def startup_event():
    ...
    # Initialize uptime monitoring with persistent data
