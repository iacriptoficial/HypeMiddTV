...
agent_communication:
    ...
    - agent: "main"
      message: "✅ POSITION MANAGEMENT IMPLEMENTED! Added comprehensive position management functionality that closes existing positions before opening new ones. Created functions: get_open_positions() to check existing positions, close_existing_positions() to close them with reduce_only=True orders. Integrated into forward_to_hyperliquid() flow with proper error handling. Uses IOC (Immediate or Cancel) limit orders with market-appropriate pricing to ensure positions are closed before new orders execute. This prevents conflicting positions and ensures clean strategy execution."
